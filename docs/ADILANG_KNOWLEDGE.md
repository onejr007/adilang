...
```
## [1.14.1] — 2026-08 — Web SDK bugfix: `ADILangJITEngine.bootFromInline` kini fallback ke query document-wide bila `<script type="text/adi">` tidak berada DI DALAM root — karena `adilang-build` (exporter.rs) & `core/adilang_site.build_index_html` menaruh script sebagai SAUDARA `#app` (bukan anak), sebelumnya querySelector(root) mengembalikan null → throw → halaman BOOT GAGAL dan tampil kosong tanpa teks/konten. Kini index.html hasil build benar-benar boot & me-render text (regression test via scripts/adilang_js_boot.js)
...
- Sumber situs: `core/adilang_site_gen.py` — **DIHASILKAN OLEH LLM (TANPA template)**.
  ADI berkomunikasi dengan LLM untuk menulis ADILang kreatif sesuai deskripsi user
  (+konten enriched LLM pass 1), lalu memvalidasi & meminta LLM memperbaiki bila gagal.
- Validasi dua lapis: `core/adilang_site.validate_site_source()` (tolak HTML, wajib
  blok UI, scan pola berbahaya) + `core/adilang_site.validate_js_parse()` (parser
  **asli framework ADI** via Node — memastikan situs benar-benar ter-render).
